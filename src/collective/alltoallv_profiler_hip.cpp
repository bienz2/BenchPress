...
/*******************************************************************
 *** Method : alltoallv_profile_gpu_aware(...)
 ***
 ***    max_i : int
...
*******************************************************************/ 
#ifdef GPU_AWARE
void alltoallv_profile_gpu_aware(int max_i, bool imsg)
{
    int rank, num_procs;
...
void alltoallv_profile_3step(int max_i, bool imsg)
{
    ...
    float* gpu_data;
    double time, max_time;
    hipMallocHost((void**)&cpu_send_data, max_bytes);
    hipMallocHost((void**)&cpu_recv_data, max_bytes);
    ...
    hipFreeHost(cpu_send_data);
    hipFreeHost(cpu_recv_data);
    MPI_Comm_free(&gpu_comm);
