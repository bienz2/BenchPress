...
def parse(n_nodes):
    files = glob.glob("%s/all_reduce_%d.*.out"%(prof.folder, n_nodes))
...
allreduce_4 = parse(4)
allreduce_8 = parse(8)
allreduce_16 = parse(16)
allreduce_32 = parse(32)
...
if __name__=='__main__':
    ...
    def plot_data(data, n_nodes):
        ...
        plt.save_plot("%s/%s_allreduce_%d.pdf"%(prof.folder_out, prof.computer, n_nodes))
    ...
    def plot_speedup(data, n_nodes):
        ...
        plt.save_plot("%s/%s_allreduce_speedup_%d.pdf"%(prof.folder_out, prof.computer, n_nodes))


    plot_data(allreduce_2, 2)
    plot_data(allreduce_4, 4)
    plot_data(allreduce_8, 8)
    plot_data(allreduce_16, 16)
    plot_data(allreduce_32, 32)

    plot_speedup(allreduce_16, 16)
    plot_speedup(allreduce_32, 32)
