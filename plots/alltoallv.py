...
def parse(n_nodes):
    files = glob.glob("%s/all_to_all_v_%d.*.out"%(prof.folder, n_nodes))
...
alltoallv_4 = parse(4)
alltoallv_8 = parse(8)
alltoallv_16 = parse(16)
alltoallv_32 = parse(32)
...
if __name__=='__main__':
    ...
    def plot_data(data, n_nodes):
        ...
        plt.save_plot("%s/%s_alltoallv_%d.pdf"%(prof.folder_out, prof.computer, n_nodes))
    ...
    def plot_speedup(data, n_nodes):
        ...
        plt.save_plot("%s/%s_alltoallv_speedup_%d.pdf"%(prof.folder_out, prof.computer, n_nodes))


    plot_data(alltoallv_2, 2)
    plot_data(alltoallv_4, 4)
    plot_data(alltoallv_8, 8)
    plot_data(alltoallv_16, 16)
    plot_data(alltoallv_32, 32)

    plot_speedup(alltoallv_16, 16)
    plot_speedup(alltoallv_32, 32)
