...
float timePingPong(int local_rank, int n_local, float* data, int size, bool active, int n_tests = 1000)
{
    int rank;
    ...
    int local_size = size / n_local;

    int proc;
    if (local_rank < n_local && active)
    {
        if (rank % 2 == 0)
        {
            first = true;
    ...
    for (int i = 0; i < n_tests; i++)
    {
        if (first)
        {
            MPI_Send(data, local_size, MPI_FLOAT, proc, ping_tag, MPI_COMM_WORLD);
            MPI_Recv(data, local_size, MPI_FLOAT, proc, pong_tag, MPI_COMM_WORLD, &status);
        }
        else if (second)
        {
            MPI_Recv(data, local_size, MPI_FLOAT, proc, ping_tag, MPI_COMM_WORLD, &status);
            MPI_Send(data, local_size, MPI_FLOAT, proc, pong_tag, MPI_COMM_WORLD);
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    int proc_counts[8] = {1,2,3,4,8,12,16,18};
    ...
    for (int i = 0; i < max_i; i++)
    {
        ...
        for (int n = 0; n < 8; n++)
        {
            time = timePingPong(socket_rank, proc_counts[n], x, s, true);
...
/*
    ...
    for (int i = 0; i < max_i; i++)
    {
        ...
        {
            time = timePingPong(socket_rank, n+1, x, s, gpu < num_gpus);
            if (rank == 0) printf("%2.5f\t", time);
        }
        if (rank == 0) printf("\n");
    }
