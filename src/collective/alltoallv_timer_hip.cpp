...
double time_alltoallv_3step(int size, float* cpu_send_data, float* cpu_recv_data,
        float* gpu_data, hipStream_t& stream, MPI_Comm& group_comm, int n_tests)
{
    int rank, num_procs;
    ...
    for (int i = 0; i < n_tests; i++)
    {
        hipMemcpyAsync(cpu_send_data, gpu_data, bytes, hipMemcpyDeviceToHost, stream);
        hipStreamSynchronize(stream);
        ...
        hipMemcpyAsync(gpu_data, cpu_recv_data, bytes, hipMemcpyHostToDevice, stream);
        hipStreamSynchronize(stream);
    }
    tfinal = (MPI_Wtime() - t0) / n_tests;
...
double time_alltoallv_3step_msg(int size, float* cpu_send_data, float* cpu_recv_data,
       float* gpu_data, int ppg, int node_rank, hipStream_t& stream, MPI_Comm& group_comm, 
       int n_tests)
{
    int rank, num_procs;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
 ...
 ***    time cost at setup.   Each CPU core calls hipMemcpyAsync on its
 ***    individual hipStream.
*******************************************************************/ 
